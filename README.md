...
```text
Project_Root/
├── install_env.bat    # [步骤1] 一键环境安装脚本 (首次必须运行)
├── run_app.bat        # [步骤2] 程序启动脚本 (日常使用)
├── main.py            # 程序主入口 (负责逻辑引导)
├── src/               # 源代码目录
│   └── player_core.py # 播放器核心逻辑、GUI与推理引擎
├── models/            # 存放自动下载的 AI 模型 (.pth 文件)
├── lib/               # 核心依赖库 (iw3, nunif, dxcam)
└── venv/              # Python 虚拟环境 (由 install_env.bat 自动生成)
...
```text
Project_Root/
├── install_env.bat    # [Step 1] One-click environment installation script (must be run the first time)
├── run_app.bat        # [Step 2] Program startup script (for daily use)
├── main.py            # Program main entry point (responsible for logic control)
├── src/               # Source code directory
│   └── player_core.py # Player core logic, GUI, and inference engine
├── models/            # Stores automatically downloaded AI models (.pth files)
├── lib/               # Core dependency libraries (iw3, nunif, dxcam)
└── venv/              # Python virtual environment (automatically generated by install_env.bat)
```
